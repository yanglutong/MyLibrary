...
public final class PermissionUtil {
    ...
    public static void handleRequestPermissionsResult(@NonNull Activity activity, int requestCode, @NonNull String[] permissions, int[] grantResults) {
        ...
        if (mRequestList != null && mRequestList.size() > 0) {
            LogUtil.d("PermissionUtil","进来了mRequestList != null && mRequestList.size() > 0"+"\r\n");
            ...
            for (PermissionRequest request : mRequestList) {
                ...
                if ((requestCode == request.getRequestCode()) && permissions.equals(request.getPermissions())) {
                    currentReq = request;
                    LogUtil.d("PermissionUtil","进来了(requestCode == request.getRequestCode()) && permissions.equals(request.getPermissions())"+"\r\n");
                    break;
                }
            }
            if (currentReq != null) {
                if (currentReq.getPermissionListener() != null) {
                    LogUtil.d("PermissionUtil","currentReq.getPermissionListener() != null"+"\r\n");
                    // 创建被拒绝授权的权限列表
                    ...
                    if (deniedPermissions.size() <= 0) {
                        ...
                        if (currentReq.getPermissionListener() != null) {
                            ...
                            LogUtil.d("PermissionUtil","// 如果所有权限都授权成功,调用授权成功的回调"+"\r\n");
                        ...
                        if (currentReq.getPermissionListener() != null) {
                            // 调用授权失败的回调
                            LogUtil.d("PermissionUtil","调用授权失败的回调"+"\r\n");
